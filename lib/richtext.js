...
function linkedRecordHref(record) {
  if (!record) return '#';
  if (record.__typename === 'BlogPostRecord') return `/blog/${record.slug}`;
  if (record.__typename === 'FileCutPasteBlogRecord') return `/FileCutPasteApp/blog/${record.slug}`;
  if (record.__typename === 'MerchandiseRecord') return `/shop/${record.slug}`;
  return '#';
}
...
export function renderRichText(field) {
  ...
  return render(field, {
    ...
    renderLinkToRecord: ({ record, children, adapter }) =>
      adapter.renderNode('a', { href: linkedRecordHref(record) }, children),
    renderInlineRecord: ({ record, adapter }) =>
      adapter.renderNode('a', { href: linkedRecordHref(record) }, record.title || ''),
  }) || '';
}
